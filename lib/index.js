  ...
  static streamify(bar, actor, opts) {
    ...
    const buildSpeedWith = speed => (_, data) =>
      `${xbytes.createRelativeSizer((data && (data.args[0] || data.matched.unit)) || defaultUnit, {
        iec: data && 'iec' in data.matched ? data.matched.iec === 'true' : !1,
        bits: data && 'bits' in data.matched ? data.matched.bits === 'true' : !0,
        fixed: data && 'fixed' in data.matched ? parseInt(data.matched.fixed, 10) : 2,
        short: data && 'short' in data.matched ? data.matched.short === 'true' : !0,
        space: data && 'space' in data.matched ? data.matched.space === 'true' : !1,
        sticky: data && 'sticky' in data.matched ? data.matched.sticky === 'true' : !1,
      })(speed)}${(data && data.matched.metric) || 'ps'}`;
