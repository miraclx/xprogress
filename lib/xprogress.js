...
class ProgressBar {
  ...
  value(index, value, template) {
    ...
    if (arguments.length > 1 && value && typeof value !== 'object')
      res = [index, parse(this.slots[index].max, value, (value * 100 ** 2) / (this.slots[index].level * this.total()))];
    else
      [template, res] = [
        value,
        [
          Array.isArray(index)
            ? this.slots
                ...
                .map((v, i) => parse(v.max, index[i], (index[i] * 100 ** 2) / (v.level * this.total())))
            : parse(this.total(), index, (index / this.total()) * 100),
        ],
      ];
  ...
  static stream(total, slots, opts, actor) {
    ...
    const progressBar = new this(total, slots, opts);
