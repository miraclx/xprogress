...
function pulsateBar(bar, slots, skip, sep = '') {
  ...
  stack[stack.length - 1].fillable += total - stack.reduce((max, {fillable}) => max + fillable, 0);
    ...
    const streamGenerator = bar.slotStreamify((slotIndex, total, infinite) => {
      ...
      const through = progressStream({length: total, ...bar.opts.progress})
        ...
        .on('progress', progress => {
          const xpeed = speed(progress.delta);
          ...
          (actor || ((_bar, args, template) => _bar.value(...args).draw(template)))(
            ...
            {
              eta: () => xprettyMs((1000 * bar.average().remaining) / xpeed),
              size: () => xbytes(bar.average().completed, {short: !0}),
              speed: (_, unit) => {
                if (!(unit && xbytes.parseUnit(unit))) unit = 'Mb'; // default to Megabits
                return `${xbytes.createRelativeSizer(unit)(xpeed, {short: !0, space: !1})}ps`;
              },
              'speed:raw': xpeed,
              progress,
              'eta:raw': () => Math.round(bar.average().remaining / xpeed),
              'slot:bar': () =>
