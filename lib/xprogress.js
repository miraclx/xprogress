...
const streamOpts = {
  ...
  template: [
    ':{label} |:{slot:bar}| [:3{slot:percentage}%] :{slot:eta} [:{slot:size}/:{slot:size:total}]',
...
class ProgressBar {
  ...
  constructor(total, arr = [100], opts = {}) {
    ...
    if (typeof arr === 'object' && !Array.isArray(arr)) [arr, opts] = [[100], arr];
    this.opts = {
      ...defaultOptions,
      ...opts,
    ...
    const self = this;
    this.slots = padRatio(arr, 100, {fixed: 15, blot: this.opts.blot, append: this.opts.append}).map(level => ({
      level,
      value: 0,
      get max() {
    ...
    const bars = !this.opts.pulsate
      ? !this.opts.forceFirst
        ? (() => {
            ...
            const slotting = this.slots.map(({level, value}) => ({portion: ((level / 100) * total) | 0, value}));
            ...
            const result = slotting.map(({portion, value}) => parseBar(this, portion, value));
            return result;
          })()
        : [
  ...
  isComplete(slot) {
    ...
    return slot ? this.slots[slot].value === 100 : this.average(20).percentage === 100;
  }
  ...
  static stream(total, slots, opts, actor) {
    ...
    const progressBar = new this(total, slots);
    ...
    const streamGenerator = bar.slotStreamify((slotIndex, total, infinite) => {
      ...
      const through = progressStream({length: total, ...bar.opts.progress})
        .on('progress', progress => {
          ...
          (actor || ((_bar, args, template) => _bar.value(...args, template)))(
            bar,
            bar.opts.progress.infinite ? [progress.delta + bar.average().completed] : [slotIndex, progress.transferred],
            {
              eta: () => moment.duration((1000 * bar.average().remaining) / progress.speed).humanize(),
              'eta:raw': () => Math.round(bar.average().remaining / progress.speed),
              size: () => xbytes(bar.average().completed, 2, {shorten: !0}),
              progress,
              'slot:bar': () =>
              ...
              'slot:eta': moment.duration(1000 * progress.eta).humanize(),
              'slot:eta:raw': progress.eta,
              'slot:size': xbytes(progress.transferred, 2, {shorten: !0}),
              'size:total': () => xbytes(bar.total(), 2, {shorten: !0}),
              'slot:runtime': moment.duration(1000 * progress.runtime).humanize(),
              'slot:runtime:raw': progress.runtime,
              'slot:percentage': progress.percentage.toFixed(0),
              'slot:size:total': xbytes(progress.length, 2, {shorten: !0}),
            },
          );
