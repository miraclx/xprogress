...
const globOpts = {
  ...
  length: () => Math.floor(getPersistentStdout().columns / 2 - 20),
  flipper: ['|', '/', '-', '\\'],
  pulsate: !1,
  template: '',
  variables: {
...
function pulsateBar(bar, slots, skip) {
  ...
  return ((stack.slice(-1).pop().fillable += total - stack.reduce((max, {fillable}) => max + fillable, 0)),
  stack).map(({fillable, percentage}) => parseBar(bar.opts, fillable, percentage, false));
}
...
class ProgressBar {
  ...
  constructBar(template) {
    const forcedFirst = [
      parseBar(this.opts, this.length() - this.opts.bar.header.length, this.average().percentage),
    ...
    const bars = !this.opts.pulsate
      ? !this.opts.forceFirst
        ? (() => {
            ...
            const slotting = this.slots.map(({level, percentage}) => ({portion: Math.floor((level / 100) * total), percentage}));
            const slack = total - slotting.reduce((max, {portion}) => max + portion, 0);
            if (slack > total / 5 || total < 0) {
              const er = new Error(
                `Resultant bar length is less than max bar length, consider increasing the size of your terminal or the bar length${(stacks =>
                  stacks.map((item, index, all) => `${index + 1 === all.length ? ' or ' : ', '}${item}`).join(''))([
                  this.opts.bar.separator ? 'removing the separator' : '',
                  this.opts.bar.header ? 'removing the headers' : '',
                  !this.opts.forceFirst ? 'using the `forceFirst` option' : '',
                ])}.`,
              );
              throw er;
            }
      ...
      const through = progressStream({length: total, ...bar.opts.progress})
        ...
        .on('progress', progress => {
          ...
          (actor || ((_bar, args, template) => _bar.value(...args).draw(template)))(
            ...
            {
              eta: () => xprettyMs((1000 * bar.average().remaining) / progress.speed),
              size: () => xbytes(bar.average().completed, {short: !0}),
              speed: () => `${xbytes(progress.speed, {bits: !0, short: !0, space: !1})}ps`,
              progress,
        ...
        .on('end', () => (bar.isComplete() ? result.emit('complete', bar) : null, result.emit('end', bar)))
